...
plt.xlabel("Time (T_b)")
plt.ylabel("Voltage")
plt.title("Unipolar NRZ Encoding: Bit Stream 10110001")
...
for i, bit in enumerate(bit_stream):
    plt.text(i * Tb + Tb / 2, 1.1, str(bit), ha='center', va='bottom', fontsize=12)

plt.ylim(-0.2, 1.4)
plt.xlim(0, len(bit_stream) * Tb)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("note_10.png", dpi=200)
plt.show()